...
// LUGAR CORRECTO: La variable se declara aquí, para que quede DENTRO de la clase Parser.
...
private int yylex() {
    // 1. Llama al yylex() del léxico para que haga su trabajo.
    int tokenId = lexer.yylex();

    // 2. Copia el valor del yylval del léxico al yylval de ESTA clase (el parser).
    // Con esta línea, el valor "cruza el puente".
    this.yylval = lexer.yylval;

    // 3. Retorna el ID del token, como antes.
    return tokenId;
}
